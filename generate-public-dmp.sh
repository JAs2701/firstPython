...
do
	...
	if [ $count -eq 0 ]
	then
		java -Xmx6g update --data $data --update $query --dump >> $output_dir/public-dump.ttl
		count=$((count + 1))		
	else
		java -Xmx6g update --data $output_dir/public-dump.ttl --update $query --dump >> $output_dir/tmp_public-dump.ttl
		# delete file
